  ...
  return (
    <div className='finished-goods-container'>
      <div className="raw-material-add-div">
        ...
        {btnBack.map((item) => (
          <Button variant="contained" key={item.id} onClick={() => navigate('/stock')}>{item.back}</Button>
        ))}
      </div>
      ...
      <div style={{ height: 470, width: '100%' }} className='raw-material-form-div'>
        <DataGrid
          ...
          rows={materialFormData.filter((row) => row.type === 'Finished Goods')}
